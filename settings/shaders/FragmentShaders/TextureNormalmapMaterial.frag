...
void main(void)
{
    ...
    NormalOut = vec4(normalize( inverseSceneTangentMatrix * ( (2 * texture(normalMapTexture, fragTexCoord).xyz) - vec3(1.0, 1.0, 1.0) ) ), 1.0); //vec4(texture(normalMapTexture, fragTexCoord).xyz , 1.0);//vec4(fragNormal, 1.0);
