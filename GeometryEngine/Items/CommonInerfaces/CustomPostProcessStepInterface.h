...
namespace GeometryEngine
{
	...
	namespace CustomShading
	{
		...
		public:
			...
			template <typename T > CustomPostProcessStep* AddNewPostProcessStep(CustomPostProcessSteps step) { return new T(this, step); }
			...
			bool AddPostProcessStep(CustomPostProcessStep* value, CustomPostProcessSteps key);
			/// Removes custom step from the manager
			/// \param key Custom step key to be removed
			bool RemovePostProcessStep(CustomPostProcessSteps key);
			/// Checks if the manager contains a custom step key
			/// \param step Key to check
			bool ContainsStep(CustomPostProcessSteps step) { return mStepMap.find(step) != mStepMap.end(); }
			...
			virtual bool RenderStep(CustomPostProcessSteps step, const GBufferTextureInfo& gBuffTexInfo);
			...
			bool InitCustomStep(CustomPostProcessSteps key);
